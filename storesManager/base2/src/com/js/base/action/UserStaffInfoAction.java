...
import com.js.base.vo.UserStaffInfo;
import com.js.commons.paginator.Page;

@Controller
@RequestMapping("/staff")
public class UserStaffInfoAction {
	private static Logger logger = Logger.getLogger(UserStaffInfoAction.class);
	@Autowired
	@Qualifier("usersService")
	private IUsersService usersService;

	@RequestMapping(value = "/userStaffList", method = { RequestMethod.GET,
			RequestMethod.POST })
	public @ResponseBody
	JSONObject userQueryList(Model model, HttpServletRequest req)
			throws Exception {
		UserStaffInfo parm = new UserStaffInfo();
		Page page = new Page();
...
//		List<UserStaffInfo> list = usersService.getUser(parm);

		JSONArray json = new JSONArray();
//		for (UserStaffInfo user : list) {
